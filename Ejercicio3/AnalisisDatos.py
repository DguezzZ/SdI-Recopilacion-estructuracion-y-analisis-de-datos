...
def numCorreos(rango: int):
    if rango < 0:
        rango = -rango
        dataframe = pd.read_sql_query("SELECT * FROM usuarios WHERE emails_total >= \'{}\' group by id".format(rango), conexion)

        dataframe["IPs"] = pd.read_sql_query("SELECT COUNT(ip) FROM ips INNER JOIN usuarios USING(username) WHERE emails_total "">= \'{}\' group by id".format(rango), conexion)

        dataframe["Fechas"] = pd.read_sql_query("SELECT COUNT(fecha) FROM fechas INNER JOIN usuarios USING(username) WHERE ""emails_total >= \'{}\' group by id".format(rango), conexion)
...
def permisos(permisos: int):
    if permisos == 1:
        dataframe = pd.read_sql_query("SELECT * FROM usuarios WHERE permisos = \'{}\' group by id".format(permisos), conexion)

        dataframe["IPs"] = pd.read_sql_query("SELECT COUNT(ip) FROM ips INNER JOIN usuarios USING(username) WHERE permisos ""= \'{}\' group by id".format(permisos), conexion)

        dataframe["Fechas"] = pd.read_sql_query("SELECT COUNT(fecha) FROM fechas INNER JOIN usuarios USING(username) WHERE ""permisos = \'{}\' group by id".format(permisos), conexion)
